# the usage text
usage="$(basename "$0") [-h] [-i <server> -r <remote-port> -u <username> -s <source-port> -d <destination-port>] -- tool untuk port forwarding
...
contoh:
    $ bash start-port-forward.sh -i 100.200.300.400 -u ur-ssh-user
    $ bash start-port-forward.sh -i 100.200.300.400 -u ur-ssh-user -r 22
    $ bash start-port-forward.sh -i 100.200.300.400 -u ur-ssh-user -s 8001 -d 8000
    $ bash start-port-forward.sh -i 100.200.300.400 -u ur-ssh-user -r 22 -s 8001 -d 8000
"

# read the input arguments
while getopts ':hi:r:u:s:d:' option; do
  case "$option" in
    h) echo "$usage"
       exit
       ;;
